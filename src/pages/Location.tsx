...
const Location: React.FC = () => {
  ...
  const handleOpenLocationModal = (shift: Shift) => {
    setModalData({
      isOpen: true,
      startLocation: shift.startLocation,
      endLocation: shift.endLocation,
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl text-primary">Shift Locations</CardTitle>
            <CardDescription>
                  ...
                  {paginatedShifts.length === 0 ? (
                    ...
                    paginatedShifts.map((shift, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          {shift.checkOut ? format(new Date(shift.checkOut), 'hh:mm a') : '-'}
                        </TableCell>
                        ...
                        <TableCell>
                          <Button 
                            variant="ghost" 
                            size="sm" 
                            onClick={() => handleOpenLocationModal(shift)}
                        ...
                        <TableCell className="max-w-[150px] truncate" title={shift.notes}>
                  ...
                  {Array.from({ length: Math.min(5, totalPages) }).map((_, i) => {
                    ...
                    if (totalPages > 5) {
                      if (currentPage > 3 && currentPage < totalPages - 1) {
                        pageNumber = [1, currentPage - 1, currentPage, currentPage + 1, totalPages][i];
                      } else if (currentPage >= totalPages - 1) {
                        pageNumber = [1, totalPages - 3, totalPages - 2, totalPages - 1, totalPages][i];
                      }
                    }
